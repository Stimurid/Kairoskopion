...
| §6.8 JournalModel | Serial journal entity | Implemented | `schema.py::JournalModel` (VF-C3); linked from VenueProfilePackage.journal_model_id | Population logic deferred to VF-C4 builder extension | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.9 SectionModel | Journal section/article type | Implemented | `schema.py::SectionModel` (VF-C3); linked from VenueProfilePackage.section_model_ids[] | Population logic deferred to VF-C4 | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.10 IssueModel/SpecialIssueModel | Time-bound containers | Implemented | `schema.py::SpecialIssueModel` (VF-C3); linked from VenueProfilePackage.special_issue_model_ids[] | Population logic deferred; CFP discovery requires VF-C8 adapters | P2 | Venue Funnel v1 (VF-C3 ✓) |
...
| §6.16 TacitVenueSignal | Non-formal knowledge | Implemented | `schema.py::TacitVenueSignal` (VF-C3); linked from VenueProfilePackage.tacit_venue_signal_ids[]; authority=`tacit_signal` enforced | Ingest path (adapter J) deferred to VF-C8 | P3 | Venue Funnel v1 (VF-C3 ✓; VF-C8 adapter) |
| §6.X-1 MethodExpectationProfile | Method/empirical stance expectation (canon §4) | Implemented | `schema.py::MethodExpectationProfile` (VF-C3); linked from VenueProfilePackage.method_expectation_profile_id | Population logic deferred to VF-C4/C5 | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.X-2 GenreMoveProfile | Argument-move distribution (canon §4) | Implemented | `schema.py::GenreMoveProfile` (VF-C3); linked from VenueProfilePackage.genre_move_profile_id | Population logic deferred to VF-C4/C5 | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.X-3 StyleRegisterProfile | Language register / jargon density | Implemented | `schema.py::StyleRegisterProfile` (VF-C3); linked from VenueProfilePackage.style_register_profile_id | Population logic deferred to VF-C4 | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.X-4 AuthorEligibilityProfile | §6.15 sibling axis (who may publish) | Implemented | `schema.py::AuthorEligibilityProfile` (VF-C3); linked from VenueProfilePackage.author_eligibility_profile_id | Population logic deferred to VF-C4 (parsing) | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.X-5 TimeReviewProfile | Decision time / review cadence | Implemented | `schema.py::TimeReviewProfile` (VF-C3); linked from VenueProfilePackage.time_review_profile_id | Population logic deferred to VF-C4 + tacit-signal ingest | P2 | Venue Funnel v1 (VF-C3 ✓) |
| §6.X-6 APCAccessProfile | APC / OA / deposit / license terms | Implemented | `schema.py::APCAccessProfile` (VF-C3); linked from VenueProfilePackage.apc_access_profile_id | Population logic deferred to VF-C4 (guidelines extractor extension) | P2 | Venue Funnel v1 (VF-C3 ✓) |
